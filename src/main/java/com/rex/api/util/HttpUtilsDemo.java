...
public class HttpUtilsDemo {
    ...
    public static void main(String[] args) throws IOException {
        //请求 uploadUrl
        String uploadUrl = "http://localhost:31001/publicFiles/v1/upload";
        InputStream inputStream = new ByteArrayInputStream(StandardCharsets.UTF_16.encode(uploadUrl).array());
        System.out.println(doPost(uploadUrl, "abc.csv", 1, inputStream));

    }
    /*
     * Author lzw
     * Description 请求调用上传文件
     * Date 2021/9/7
     * Param [url 请求地址, fileName 文件名, isOverwrite 是否覆盖, inputStream 输入流]
     * return com.parameter.query.entity.HttpResponse
     **/
    public static HttpResponse doPost(String url, String fileName, int isOverwrite, InputStream inputStream) throws IOException {
        //请求 uploadUrl

        // keyValues 保存普通参数
        Map<String, Object> keyValues = new HashMap<>();
        keyValues.put("fileName", fileName);
        keyValues.put("isOverwrite",isOverwrite);

        // filePathMap 保存文件类型的参数名和文件路径
        String tempname = "tmpfile";//tempfile代表临时存放文件
        ...
        int n;
        while((n = inputStream.read(b))!= -1) {//从输出流中每次读取1024字节,直至读完
        ...
        if (tempfile.exists()){
            System.out.println(tempfile.delete());//删除临时文件
        }
        outputStream.close();
    ...
    public HttpResponse sendPost(String url, String fileName, int isOverwrite, InputStream file) throws IOException {
